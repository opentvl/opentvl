...
async function tvl(_, block) {
  const lpBalance = await getLPBalance(block);
  const totalLPSupply = await getTotalLPSupply(block);
  const percentPoolOwnership = (new BigNumber(lpBalance)).dividedBy(new BigNumber(totalLPSupply));

  const pairAddresses = [SAFE_MOON_LP];
  let balances = Object.entries(await sdk.bsc.swap.getReservedBalances(pairAddresses))
